...
def init_model(train=True):
    ...
    color_image_yuv = rgb_to_yuv(color_image_rgb, scope="rgb_to_yuv_color_image")
    ...
    gray_image_yuv = rgb_to_yuv(gray_image_rgb, scope="rgb_to_yuv_gray_image")
    ...
    predict_rgb = yuv_to_rgb(predict_yuv, scope="yuv_to_rgb_pred_image")
    ...
    lr = tf.div(learning_rate, tf.cast(tf.pow(2, tf.div(global_step, 5000)), tf.float32), name="learning_rate")
    ...
    tf.histogram_summary("cost", tf.reduce_mean(cost), name="summary_cost")
    tf.histogram_summary("learning_rate", lr, name="summary_learning_rate")
    tf.image_summary("gray_image", gray_image_rgb, max_images=1, name="summary_gray_rgb")
    tf.image_summary("predict_image", predict_rgb, max_images=1, name="summary_pred_rgb")
    tf.image_summary("color_image", color_image_rgb, max_images=1, name="summary_color_rgb")
